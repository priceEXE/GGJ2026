...
namespace MemoFramework
{
    ...
    public struct MFLinkedListRange<T> : IEnumerable<T>, IEnumerable
    {
        ...
        public MFLinkedListRange(LinkedListNode<T> first, LinkedListNode<T> terminal)
        {
            if (first == null || terminal == null || first == terminal)
            {
                throw new MFException("Range is invalid.");
            }
        ...
        /// <returns>链表范围中指定索引处的元素。</returns>
        /// <exception cref="InvalidOperationException">当链表范围无效时抛出。</exception>
        ...
        public T this[int index]
        {
            get
            {
                if (!IsValid)
                {
                    throw new InvalidOperationException("The linked list range is invalid.");
                }
                ...
                for (int i = 0; i < index; i++)
                {
                    ...
                    // 检查是否超出范围或遇到终结节点
                    if (current == null || current == m_Terminal)
                    {
        ...
        {
            private readonly MFLinkedListRange<T> _mMfLinkedListRange;
            private LinkedListNode<T> m_Current;
            private T m_CurrentValue;

            internal Enumerator(MFLinkedListRange<T> range)
            {
                if (!range.IsValid)
                {
                    throw new MFException("Range is invalid.");
                ...
                _mMfLinkedListRange = range;
                m_Current = _mMfLinkedListRange.m_First;
            ...
            {
                if (m_Current == null || m_Current == _mMfLinkedListRange.m_Terminal)
                {
                    return false;
                }
